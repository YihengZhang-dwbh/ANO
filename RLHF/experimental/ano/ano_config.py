...
class ANOConfig(TrainingArguments):
    ...
    Parameters:
        ...
        kl_coef (`float`, *optional*, defaults to `0.05`):
            KL coefficient. Kept identical to the PPO/SPO default so that ANO differs from
            those baselines in the policy objective only. Any deviation used for a specific
            experiment must be passed explicitly on the command line (e.g. `--kl_coef 0.055`)
            so that it is recorded in the run configuration rather than hidden in a default.
    ...
    kl_coef: float = field(
        default=0.05,
        metadata={
            "help": "KL coefficient. Identical to the PPO/SPO default so that ANO differs from "
            "those baselines in the policy objective only. Pass --kl_coef explicitly to deviate."
        },
    )
