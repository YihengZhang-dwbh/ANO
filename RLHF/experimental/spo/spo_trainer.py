                ...
                for mini_batch_start in range(0, args.local_batch_size, args.local_mini_batch_size):
                    ...
                    for micro_batch_start in range(0, args.local_mini_batch_size, args.per_device_train_batch_size):
                        with accelerator.accumulate(model):
                            ...
                            # [OPTIMIZED CALL]
                            # NOTE: keep this per-token (no .mean() here). Reducing with .mean()
                            # averages over padding positions as well, which makes the subsequent
                            # masked_mean a no-op and silently scales the policy gradient by
                            # (num_valid_tokens / num_total_tokens) -- an effective learning-rate
                            # reduction that PPO and ANO (which reduce with masked_mean) do not get.
                            pg_loss_max = -(
                                mb_advantage * ratio
                                - torch.abs(mb_advantage) * torch.pow(ratio - 1, 2) / (2 * args.cliprange)
                            )
